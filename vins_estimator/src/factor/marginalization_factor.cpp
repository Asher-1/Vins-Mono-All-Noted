...
/**
 * @brief 待边缘化的各个残差块计算残差和雅可比矩阵，同时处理核函数的case
 */
void ResidualBlockInfo::Evaluate()
{
    ...
    // ceres接口都是double数组，因此这里给雅可比准备数组
    ...
    if (loss_function)
    {
        ...
        // 柯西核 p = log(s+1), rho[2]<=0始终成立，一般核函数二阶导数都是小于0
        ...
        // 这里就相当于残差雅可比都乘上sqrt_rho1，及核函数所在的点的一阶导，基本都是小于1
        for (int i = 0; i < static_cast<int>(parameter_blocks.size()); i++)
        {
...
/**
 * @brief 将各个残差块计算残差和雅可比，同时备份所有相关的参数块内容
 */
...
void* ThreadsConstructA(void* threadsstruct)
{
    ...
    for (auto it : p->sub_factors)
    {
        ...
        for (int i = 0; i < static_cast<int>(it->parameter_blocks.size()); i++)
        {
            ...
            if (size_i == 7)
                size_i = 6;
            // 之前pre margin 已经算算好了各个残差和雅可比，这里取出来
...
void MarginalizationInfo::marginalize()
{
    ...
    */
    ...
    // 这个地方根据Ax = b => JT*J * deltaX = -JT * e --> b = -JT * e --> e = -(JT)-1 * b
    // 对A做特征值分解 A = V * S * VT, 其中S是特征值构成的对角矩阵
    // 令J = S^(1/2) * VT
    // 得 JT * J = (S^(1/2) * VT)T * S^(1/2) * VT = V * S^(1/2)T * S^(1/2) * VT = V * S * VT(对角矩阵的)
    // 因为 (S^(1/2))T = S^(1/2)和VT = V^-1
    ...
    Eigen::VectorXd S_sqrt = S.cwiseSqrt(); // 这个求得就是S^(1/2)，不过这里是向量还不是矩阵
...
bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // FEJ 为了解决边缘化把不客观的量变成可观的问题， 而且边缘化掉的雅可比不能更新
    // 更新残差 边缘化后的先验误差 e = e0 + J * dx
    // 个人理解：根据FEJ 雅可比保持不变，但是残差随着优化会变化，因此下面不更新雅可比 值更新残差
    // 关于FEJ的理解可以参考https://www.zhihu.com/question/52869487
    // 下面这段代码过程可以参考 https://blog.csdn.net/weixin_41394379/article/details/89975386
